        ...
        for device in testbed:
            ...
            with steps.start('Store data',continue_=True) as step:
                ...
                if self.parsed_show_access_lists is not None:
                    ...
                    with open("Cave_of_Wonders/Cisco/IOS_XE/Learned_VLAN/%s_show_access_lists_netgraph.json" % device.alias, "w") as fh:
                        fh.write(parsed_output_netjson_json)
                        fh.close()               

                    with open("Cave_of_Wonders/Cisco/IOS_XE/Learned_VLAN/%s_show_access_lists_netgraph.html" % device.alias, "w") as fh:
                        fh.write(parsed_output_netjson_html)
                        fh.close()
...
# ----------------
    def save_to_json_file(self, device, directory, file_name, content):
        file_path = "Cave_of_Wonders/Cisco/IOS_XE/{}/{}_{}.json".format(directory, device.alias, file_name)
        with open(file_path, "w") as json_file:
            json.dump(content, json_file, indent=4, sort_keys=True)
            json_file.close()
    
    def save_to_yaml_file(self, device, directory, file_name, content):
        file_path = "Cave_of_Wonders/Cisco/IOS_XE/{}/{}_{}.yaml".format(directory, device.alias, file_name)
    ...
    def save_to_specified_file_type(self, device, directory, file_name, content, file_type):
        file_path = "Cave_of_Wonders/Cisco/IOS_XE/{}/{}_{}.{}".format(directory, device.alias, file_name, file_type)
