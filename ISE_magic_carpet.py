...
class Collect_Information(aetest.Testcase):
    ...
    def parse(self, section, steps):
        ...
        for device in testbed['devices']:
            ...
            with open("Cave_of_Wonders/Cisco/ISE/Endpoints/endpoint_details.csv",'a') as csv:
                csv.seek(0, 0)
                ...
                csv.close()  
            with open("Cave_of_Wonders/Cisco/ISE/Endpoints/endpoint_details.md",'a') as md:
                md.seek(0, 0)
                ...
                md.write("| ---  | ----------- | -------- | ------ | -------------- | ----------------- | ----------- | ---------- | ------------ | -------------- |")
                md.close()
            with open("Cave_of_Wonders/Cisco/ISE/Endpoints/endpoint_details.html",'a') as html:
                html.seek(0, 0)
            ...
            for page in range(1,math.ceil(pagecount/100+1)):
                ...
                for device in self.raw_endpoints.json()['SearchResult']['resources']:
                    ...
                    with steps.start('Store data',continue_=True) as step:
                        ...
                        with open("Cave_of_Wonders/Cisco/ISE/Endpoints/endpoint_details.json", "a") as fid:
                            json.dump(self.raw_endpoint_details.json(), fid, indent=4, sort_keys=True)
                            fid.write('\n')
                                
                        with open("Cave_of_Wonders/Cisco/ISE/Endpoints/endpoint_details.yaml", "a") as yml:
                            yaml.dump(self.raw_endpoint_details.json(), yml, allow_unicode=True)
                        ...
                        for filetype in filetype_loop:
                            ...
                            with open("Cave_of_Wonders/Cisco/ISE/Endpoints/endpoint_details.%s" % filetype, "a") as fh:
                                fh.write(parsed_endpoint_details)
                                fh.close()
            ...
            if os.path.exists("Cave_of_Wonders/Cisco/ISE/Endpoints/endpoint_details.md"):
                os.system("markmap --no-open Cave_of_Wonders/Cisco/ISE/Endpoints/endpoint_details.md --output Cave_of_Wonders/Cisco/ISE/Endpoints/endpoint_details_mind_map.html")
            fid.close()
            yml.close()
