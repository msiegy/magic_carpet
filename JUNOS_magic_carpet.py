    ...
    def parse(self, testbed, section, steps):
        ...
        for device in testbed:
            ...
            with steps.start("Store data", continue_=True) as step:
                ...
                os.system(f"markmap Cave_of_Wonders/Juniper/Show_System_Information/{device.alias}_show_system_information.md --output Cave_of_Wonders/Juniper/Show_System_Information/{device.alias}_show_system_information.html")

                # Show chassis hardware
                if hasattr(self, "parsed_chassis_hardware"):
                    sh_chassis_hardware_csv_template = env.get_template("show_chassis_hardware.j2")

                with open(f"Cave_of_Wonders/Juniper/Show_Chassis_Hardware/{device.alias}_show_chassis_hardware.json", "w") as fid:
                    json.dump(self.parsed_chassis_hardware, fid, indent=4, sort_keys=True)

                with open(f"Cave_of_Wonders/Juniper/Show_Chassis_Hardware/{device.alias}_show_chassis_hardware.yaml", "w") as yml:
                ...
                for filetype in filetype_loop:
                    ...
                    with open(f"Cave_of_Wonders/Juniper/Show_Chassis_Hardware/{device.alias}_show_chassis_hardware.{filetype}", "w") as fh:
                        fh.write(parsed_output_type)

                os.system(f"markmap Cave_of_Wonders/Juniper/Show_Chassis_Hardware/{device.alias}_show_chassis_hardware.md --output Cave_of_Wonders/Juniper/Show_Chassis_Hardware/{device.alias}_show_chassis_hardware.html")
