...
if os.path.exists("Inventory/Inventory.json"):
    os.remove("Inventory/Inventory.json")

if os.path.exists("Inventory/Inventory.yaml"):
    os.remove("Inventory/Inventory.yaml")

if os.path.exists("Inventory/Inventory_DB.json"):
    os.remove("Inventory/Inventory_DB.json")       

if os.path.exists("Inventory/Contracts.json"):
    os.remove("Inventory/Contracts.json")

if os.path.exists("Inventory/Contracts.yaml"):
    os.remove("Inventory/Contracts.yaml")

if os.path.exists("Inventory/Contracts_DB.json"):
    os.remove("Inventory/Contracts_DB.json")

inventory_db = TinyDB('Inventory/Inventory_DB.json')
contract_db = TinyDB('Inventory/Contracts_DB.json')

# ----------------
# Load Credentials 
# ----------------

if os.path.exists("testbed/Serial_Sultan_f5_testbed.yaml"):
    with open("testbed/Serial_Sultan_f5_testbed.yaml") as stream:
...
class Collect_Information(aetest.Testcase):
    ...
    def parse(self, testbed, section, steps):
        ...
        # ---------------------------------------
        with open("Inventory/Inventory.csv",'w') as csv:
            csv.seek(0, 0)
            ...
            csv.close()  
        with open("Inventory/Inventory.md",'w') as md:
            md.seek(0, 0)
            md.write("# Inventory")
            md.write("\n")
            ...
            md.write("| -------- | ---- | ---- | ----------- | ------------- | ---------- | ------- | ------------ | ------------------- | --------------------- | ------------------ |")
            md.close()
        with open("Inventory/Inventory.html",'w') as html:
            html.seek(0, 0)
        ...
        with open("Inventory/Contracts.csv",'w') as csv:
            csv.seek(0, 0)
        ...
        with open("Inventory/Contracts.md",'w') as md:
            md.seek(0, 0)
        ...
        with open("Inventory/Contracts.html",'w') as html:
            html.seek(0, 0)
            ...
            with steps.start('Store data',continue_=True) as step:
                ...
                if device.os == "nxos":
                    if self.parsed_show_interface_transceiver is not None:
                        # NXOS
                        sh_interface_transceiver_template = nxos_env.get_template('sultan_show_interface_transceiver.j2')
                    
                        with open("Inventory/Inventory.json", "a") as fid:
                            json.dump(self.parsed_show_interface_transceiver, fid, indent=4, sort_keys=True)
                            fid.write('\n')
                                
                        with open("Inventory/Inventory.yaml", "a") as yml:
                        ...
                        for filetype in filetype_loop:
                            ...
                            with open("Inventory/Inventory.%s" % filetype, "a") as fh:
                ...
                if self.parsed_show_inventory is not None:
                    # 3560
                    sh_inventory_2960_template = ios_env.get_template('sultan_show_inventory_2960.j2')

                    # 3560
                    sh_inventory_3560_template = ios_env.get_template('sultan_show_inventory_3560.j2')

                    # 3750
                    sh_inventory_3750_template = ios_env.get_template('sultan_show_inventory_3750.j2')

                    # 6500
                    sh_inventory_6500_template = ios_env.get_template('sultan_show_inventory_6500.j2')

                    # 3850
                    sh_inventory_3850_template = ios_xe_env.get_template('sultan_show_inventory_3850.j2')

                    # 4500
                    sh_inventory_4500_template = ios_xe_env.get_template('sultan_show_inventory_4500.j2')

                    # 9300
                    sh_inventory_9300_template = ios_xe_env.get_template('sultan_show_inventory_9300.j2')

                    # ISR
                    sh_inventory_isr_template = isr_env.get_template('sultan_show_inventory_isr.j2')

                    # NXOS
                    sh_inventory_nxos_template = nxos_env.get_template('sultan_show_inventory_nxos.j2')
                    ...
                    serial_2_info_template = cisco_api_env.get_template('sultan_serial_2_info.j2')
                                    ...
                                    for part,value in value.items():
                                        ...
                                        with steps.start('Calling the Cisco Serial 2 Info REST API',continue_=True) as step:
                                            try:                                      
                                                ...
                                                with open("Inventory/Contracts.json", "a") as fid:
                                                    json.dump(serial_2_info_json, fid, indent=4, sort_keys=True)
                                                    fid.write('\n')

                                                with open("Inventory/Contracts.yaml", "a") as yml:
                                                ...
                                                for filetype in filetype_loop:
                                                    ...
                                                    with open("Inventory/Contracts.%s" % filetype, "a") as fh:
            ...
            for device in f5_testbed['devices']:
                ...
                with steps.start('Store data',continue_=True) as step:
                    ...
                    if self.raw_sfp_serial is not None:
                        ...
                        # F5 SFP
                        sh_inventory_f5_sfp_template = f5_env.get_template('sultan_sfp_serial_api.j2')
                    ...
                    if self.raw_license is not None:
                        ...
                        # F5 License
                        sh_inventory_f5_license_template = f5_env.get_template('sultan_license_api.j2')
        ...
        with open("Inventory/Inventory.html", "a") as html:
            html.write("</table></body></html>")
            html.close() 
                            
        if os.path.exists("Inventory/Inventory.md"):
            os.system("markmap --no-open Inventory/Inventory.md --output Inventory/Inventory_mind_map.html")

            fh.close()
            fid.close()
            yml.close()

        if os.path.exists("Inventory/Contracts.md"):
            os.system("markmap --no-open Inventory/Contracts.md --output Inventory/Contracts_mind_map.html")
